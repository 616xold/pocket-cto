...
15. shipped F6C collections-pressure monitor result and optional alert-card posture, recorded in `plans/FP-0052-collections-pressure-monitor-foundation.md`, without a broader F6 eval dataset until a later plan defines one
16. wiki compilation quality
17. provenance, freshness disclosure, and contradiction handling
...
- use `pnpm smoke:cash-posture-monitor:local`, `pnpm smoke:cash-posture-alert-investigation:local`, and `pnpm smoke:collections-pressure-monitor:local` as the current shipped F6 truth sources; `plans/FP-0052-collections-pressure-monitor-foundation.md` records the shipped F6C implementation, and no broader F6 eval dataset should be added before a later Finance Plan defines one
