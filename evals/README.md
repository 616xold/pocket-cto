...
- `plans/FP-0048-board-packet-circulation-actor-correction-and-chronology-hardening.md` is the active F5C4H contract and now covers shipped append-only actor-attribution correction plus derived effective actor chronology on the existing `report_circulation` seam
- grade only that narrow actor-correction truth on the existing seam; do not widen grading into multi-packet circulation, actual delivery, PDF export, slide or Marp export, or runtime-codex drafting behavior
