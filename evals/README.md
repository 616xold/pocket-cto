...
- keep F5 memo and shipped board-packet evals anchored to the shipped F5A through F5C1 contracts, and keep the lender-update eval scope anchored to the shipped `FP-0039` record: one completed reporting mission with stored `finance_memo` plus stored `evidence_appendix` in, one deterministic draft `lender_update` out, with explicit freshness, limitations, review-ready posture, and no runtime-codex fact invention
