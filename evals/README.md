...
- `plans/FP-0048-board-packet-circulation-actor-correction-and-chronology-hardening.md` is the active unshipped F5C4H contract; until it lands, keep grading anchored to the shipped F5C4G baseline and treat corrected `circulatedBy` as a planned narrow follow-on rather than current product truth
- when F5C4H lands, grade only append-only actor-attribution correction and derived effective actor chronology on the existing `report_circulation` seam; do not widen grading into multi-packet circulation, actual delivery, PDF export, slide or Marp export, or runtime-codex drafting behavior
