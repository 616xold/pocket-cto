...
const DEFAULT_COMPANY_KEY = "local-receivables-aging-smoke-company";
const DEFAULT_COMPANY_NAME = "Local Receivables Aging Smoke Company";
const DEFAULT_CREATED_BY = "finance-receivables-aging-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged finance receivables-aging smoke.",
      requestedBy: "finance_receivables_aging_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    sourceName: `Receivables aging smoke ${input.runTag}`,
    seed: {
      body: Buffer.from(`${seedText}\n`, "utf8"),
      mediaType: "application/json",
      originalFileName: `finance-receivables-aging-seed-${input.runTag}.json`,
    },
    upload: {
  ...
  try {
    ...
    const created = await injectJson(app, {
      ...
      payload: {
        ...
        description: "Packaged finance receivables-aging smoke source.",
        kind: "dataset",
    ...
    const [receivablesAging, collectionsPosture] = await Promise.all([
      injectJson(app, {
        expectedStatus: 200,
        method: "GET",
        url: `/finance-twin/companies/${fixture.companyKey}/receivables-aging`,
      }),
      injectJson(app, {
        expectedStatus: 200,
        method: "GET",
        url: `/finance-twin/companies/${fixture.companyKey}/collections-posture`,
      }),
    ]);

    if (receivablesAging.customerCount !== 3) {
      throw new Error(
        `Expected three persisted receivables-aging customers, received ${receivablesAging.customerCount}`,
      );
    }

    const usdBucket = collectionsPosture.currencyBuckets.find(
      (bucket) => bucket.currency === "USD",
    );

    if (!usdBucket) {
      throw new Error("Expected a USD collections-posture bucket");
    }

    if (
      usdBucket.totalReceivables !== "200.00" ||
      usdBucket.currentBucketTotal !== "100.00" ||
      usdBucket.pastDueBucketTotal !== "100.00"
    ) {
      throw new Error(
        `Unexpected USD collections-posture totals: ${JSON.stringify(usdBucket)}`,
      );
    }

    const lineageRef = receivablesAging.rows[0]?.lineageRef ?? null;

    if (lineageRef?.targetKind !== "receivables_aging_row") {
      throw new Error(
        `Expected receivables-aging lineage ref, received ${JSON.stringify(lineageRef)}`,
      );
    }
      ...
      JSON.stringify(
        {
          ...
          company: {
            companyKey: receivablesAging.company.companyKey,
            displayName: receivablesAging.company.displayName,
          },
          source: {
            id: sourceId,
          ...
          receivablesAging: {
            ...
            latestSuccessfulSlice: receivablesAging.latestSuccessfulSlice,
            rows: receivablesAging.rows,
          },
          collectionsPosture: {
            freshness: collectionsPosture.freshness,
            ...
            limitations: collectionsPosture.limitations,
          },
        },
        null,
        2,
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-finance-receivables-aging-smoke-"),
  );
