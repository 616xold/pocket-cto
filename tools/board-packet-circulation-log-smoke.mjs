...
const DEFAULT_COMPANY_KEY = "local-board-packet-circulation-log-company";
const DEFAULT_COMPANY_NAME = "Local Board Packet Circulation Log Company";
const DEFAULT_CREATED_BY = "board-packet-circulation-log-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged F5C4F board-packet circulation-log smoke.",
      requestedBy: "board_packet_circulation_log_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    sourceName: `Board packet circulation log smoke ${input.runTag}`,
    seed: {
      body: Buffer.from(`${seedText}\n`, "utf8"),
      mediaType: "application/json",
      originalFileName: `board-packet-circulation-log-seed-${input.runTag}.json`,
    },
    upload: {
  ...
  try {
      ...
      (
        await injectJson(embeddedApp, {
          ...
          payload: {
            ...
            description:
              "Packaged F5C4F board packet circulation-log smoke source.",
            kind: "dataset",
    ...
    assert(
      finalApprovalCard?.status === "approved" &&
        ...
        listedBoardPacketMission?.circulationReadiness?.circulationReady ===
          true &&
        listedBoardPacketMission?.circulationRecord?.circulated === false,
    ...
    assert(
      finalDetail.reporting?.publication === null &&
        ...
        finalDetail.reporting?.circulationRecord?.circulated === false &&
        finalDetail.proofBundle.circulationRecord?.circulated === false &&
    ...
    assert(
      replayTypes.includes("approval.requested") &&
        ...
        !replayTypes.includes("approval.circulation_logged") &&
    ...
    const circulatedAt = new Date(
      now.getTime() + 10 * 60 * 1000,
    ).toISOString();
    const circulationNote =
      "Logged from the packaged F5C4F smoke after external board circulation.";
    ...
    const circulatedDetail = await injectJson(apiApp, {
      expectedStatus: 200,
      method: "GET",
      url: `/missions/${boardPacketDetail.mission.id}`,
    });
    const circulatedMissionList = await injectJson(apiApp, {
      expectedStatus: 200,
      method: "GET",
      url: "/missions?limit=20",
    });
    const circulatedApprovalCard =
      circulatedDetail.approvalCards.find(
        (card) => card.approvalId === requestedApproval.approvalId,
      ) ?? null;
    const circulatedMissionListItem =
      circulatedMissionList.missions.find(
        (mission) => mission.id === boardPacketDetail.mission.id,
      ) ?? null;
    const circulatedReplay = await injectJson(apiApp, {
      expectedStatus: 200,
      method: "GET",
      url: `/missions/${boardPacketDetail.mission.id}/events`,
    });
    const circulatedReplayTypes = Array.isArray(circulatedReplay)
      ? circulatedReplay.map((event) => event.type)
      : [];

    assert(
      circulatedDetail.reporting?.circulationReadiness
        ?.circulationApprovalStatus === "approved_for_circulation" &&
        circulatedDetail.reporting?.circulationRecord?.circulated === true &&
        circulatedDetail.reporting?.circulationRecord?.circulatedAt ===
          circulatedAt &&
        circulatedDetail.reporting?.circulationRecord?.circulatedBy ===
          fixture.createdBy &&
        circulatedDetail.reporting?.circulationRecord?.circulationChannel ===
          "email" &&
        circulatedDetail.proofBundle.circulationRecord?.circulated === true &&
        circulatedDetail.proofBundle.circulationRecord?.circulationChannel ===
          "email",
      "Expected mission detail and proof bundle read models to show one logged external board-packet circulation while preserving approved_for_circulation posture.",
    );
    assert(
      circulatedApprovalCard?.status === "approved" &&
        circulatedApprovalCard?.requiresLiveControl === false &&
        circulatedApprovalCard?.summary?.includes(
          "External board packet circulation is logged",
        ) &&
        circulatedMissionListItem?.circulationRecord?.circulated === true &&
        circulatedMissionListItem?.circulationRecord?.circulationChannel ===
          "email",
      "Expected approval cards and mission-list read models to surface the logged external board-packet circulation without live control.",
    );
    assert(
      circulatedDetail.reporting?.publication === null &&
        circulatedDetail.proofBundle.reportPublication === null &&
        circulatedDetail.reporting?.releaseReadiness === null &&
        circulatedDetail.proofBundle.releaseReadiness === null &&
        circulatedDetail.reporting?.releaseRecord === null &&
        circulatedDetail.proofBundle.releaseRecord === null &&
        circulatedDetail.proofBundle.circulationRecord?.circulated === true &&
        circulatedDetail.proofBundle.circulationRecord?.summary.includes(
          "External circulation was logged",
        ) &&
        circulatedDetail.proofBundle.riskSummary.includes("did not send") &&
        circulatedDetail.proofBundle.riskSummary.includes("PDF") &&
        circulatedDetail.proofBundle.riskSummary.includes("slides") &&
        circulatedDetail.proofBundle.rollbackSummary.includes(
          "operator-entered external circulation log",
        ) &&
        circulatedReplayTypes.includes("approval.circulation_logged"),
      `Expected the logged-circulation proof bundle to stay delivery-free while replay adds approval.circulation_logged. Received ${JSON.stringify(
        {
          reportingPublication: circulatedDetail.reporting?.publication ?? null,
          reportPublication: circulatedDetail.proofBundle.reportPublication ?? null,
          circulationRecord: circulatedDetail.proofBundle.circulationRecord ?? null,
          verificationSummary: circulatedDetail.proofBundle.verificationSummary,
          riskSummary: circulatedDetail.proofBundle.riskSummary,
          rollbackSummary: circulatedDetail.proofBundle.rollbackSummary,
          replayTail: circulatedReplayTypes.slice(-8),
        },
      )}`,
    );
      ...
      JSON.stringify(
        {
          ...
          mode: "api_only_persisted_report_circulation_log",
          company: {
            companyKey: fixture.companyKey,
          ...
          circulationLog: {
            circulated:
              circulatedDetail.reporting.circulationRecord.circulated,
            circulatedAt:
              circulatedDetail.reporting.circulationRecord.circulatedAt,
            circulatedBy:
              circulatedDetail.reporting.circulationRecord.circulatedBy,
            circulationChannel:
              circulatedDetail.reporting.circulationRecord.circulationChannel,
            circulationNote:
              circulatedDetail.reporting.circulationRecord.circulationNote,
            approvalCardRequiresLiveControl:
              circulatedApprovalCard.requiresLiveControl,
          },
          proofBundle: {
            status: circulatedDetail.proofBundle.status,
            verificationSummary: circulatedDetail.proofBundle.verificationSummary,
            riskSummary: circulatedDetail.proofBundle.riskSummary,
            rollbackSummary: circulatedDetail.proofBundle.rollbackSummary,
            reportPublication: circulatedDetail.proofBundle.reportPublication,
            circulationRecord: circulatedDetail.proofBundle.circulationRecord,
            releaseReadiness: circulatedDetail.proofBundle.releaseReadiness,
            releaseRecord: circulatedDetail.proofBundle.releaseRecord,
          },
          replayTail: circulatedReplayTypes.slice(-8),
        },
        null,
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-board-packet-circulation-log-smoke-"),
  );
