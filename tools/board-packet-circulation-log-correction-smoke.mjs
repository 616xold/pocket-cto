...
const DEFAULT_COMPANY_KEY = "local-board-circ-correction";
const DEFAULT_COMPANY_NAME =
  "Local Board Packet Circulation Log Correction Company";
const DEFAULT_CREATED_BY = "board-packet-circulation-log-correction-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged F5C4G board-packet circulation-log-correction smoke.",
      requestedBy: "board_packet_circulation_log_correction_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    seed: {
      ...
      originalFileName: `board-packet-circulation-log-correction-seed-${input.runTag}.json`,
    },
    sourceName: `Board packet circulation log correction smoke ${input.runTag}`,
    upload: {
...
async function main() {
  ...
  try {
      ...
      (
        await injectJson(embeddedApp, {
          ...
          payload: {
            ...
            description:
              "Packaged F5C4G board packet circulation-log-correction smoke source.",
            kind: "dataset",
    ...
    const circulationNote =
      "Logged from the packaged F5C4G smoke after external board circulation.";
    ...
    const correctedCirculatedAt = new Date(
      now.getTime() + 12 * 60 * 1000,
    ).toISOString();
    const correctedNote =
      "Corrected after finance mailbox audit confirmed the earlier send time.";
    const correctionReason =
      "Corrected the original send timestamp after mailbox review";
    const correctionKey = `board-packet-correction-${runTag.toLowerCase()}`;
    const correctedCirculation = await injectJson(apiApp, {
      ...
      payload: {
        ...
        circulationNote: correctedNote,
      },
      url: `/missions/${boardPacketDetail.mission.id}/reporting/circulation-log-correction`,
    });
    assert(
      correctedCirculation.created === true &&
        correctedCirculation.circulationRecord?.circulatedAt === circulatedAt &&
        correctedCirculation.circulationChronology?.correctionCount === 1 &&
        correctedCirculation.circulationChronology?.effectiveRecord?.circulatedAt ===
          correctedCirculatedAt,
      "Expected one append-only correction to preserve the original record while updating the effective chronology.",
    );
    ...
    assert(
      correctedDetail.reporting?.circulationRecord?.circulatedAt ===
        ...
        correctedDetail.proofBundle.circulationRecord?.circulatedAt ===
          circulatedAt,
      "Expected the original circulation record to remain immutable after correction logging.",
    );
    assert(
      correctedDetail.reporting?.circulationChronology?.correctionCount === 1 &&
        ...
        correctedDetail.proofBundle.circulationChronology?.effectiveRecord
          ?.circulatedAt === correctedCirculatedAt,
      "Expected mission detail and proof bundle read models to expose original record plus corrected effective chronology.",
    );
    assert(
      ...
      "Expected approval cards and mission-list read models to surface board-circulation correction chronology.",
    );
    assert(
      ...
      JSON.stringify(
        {
          ...
          mode: "api_only_persisted_report_circulation_log_correction",
          company: {
            companyKey: fixture.companyKey,
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-board-packet-circulation-log-correction-smoke-"),
  );
