...
const DEFAULT_COMPANY_KEY = "local-lender-update-release-approval-company";
const DEFAULT_COMPANY_NAME = "Local Lender Update Release Approval Company";
const DEFAULT_CREATED_BY = "lender-update-release-approval-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged F5C4A lender-update release-approval smoke.",
      requestedBy: "lender_update_release_approval_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    sourceName: `Lender update release approval smoke ${input.runTag}`,
    seed: {
      body: Buffer.from(`${seedText}\n`, "utf8"),
      mediaType: "application/json",
      originalFileName: `lender-update-release-approval-seed-${input.runTag}.json`,
    },
    upload: {
      ...
      (
        await injectJson(embeddedApp, {
          ...
          payload: {
            ...
            description: "Packaged F5C4A lender update release-approval smoke source.",
            kind: "dataset",
      ...
      JSON.stringify(
        {
          ...
          mode: "api_only_persisted_report_release_approval",
          company: {
            companyKey: fixture.companyKey,
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-lender-update-release-approval-smoke-"),
  );
