...
const DEFAULT_COMPANY_KEY = "local-board-circ-note-reset";
const DEFAULT_COMPANY_NAME =
  "Local Board Packet Circulation Note Reset Company";
const DEFAULT_CREATED_BY = "board-packet-circulation-note-reset-smoke";
...
function buildFixture(input) {
  const seedText = JSON.stringify(
    {
      ...
      note: "Seed snapshot for the packaged F5C4I board-packet circulation note-reset smoke.",
      requestedBy: "board_packet_circulation_note_reset_smoke",
      runTag: input.runTag,
    },
    null,
    2,
  ...
  return {
    ...
    seed: {
      ...
      originalFileName: `board-packet-circulation-note-reset-seed-${input.runTag}.json`,
    },
    sourceName: `Board packet circulation note reset smoke ${input.runTag}`,
    upload: {
...
async function main() {
  ...
  try {
      ...
      (
        await injectJson(embeddedApp, {
          ...
          payload: {
            ...
            description:
              "Packaged F5C4I board packet circulation note-reset smoke source.",
            kind: "dataset",
    ...
    const circulationNote =
      "Logged from the packaged F5C4I smoke after external board circulation.";
    ...
    const correctionReason = "Clear the mistakenly logged circulation note.";
    const correctionKey = `board-packet-note-reset-${runTag.toLowerCase()}`;
    ...
    assert(
      correctedCirculation.created === true &&
        correctedCirculation.circulationRecord?.circulationNote ===
          circulationNote &&
        ...
        correctedCirculation.circulationChronology?.latestCorrectionSummary?.includes(
          "circulationNote cleared to absent",
        ),
      "Expected one append-only note-reset correction to preserve the original record while clearing the effective note.",
    );
    ...
    assert(
      correctedDetail.reporting?.circulationRecord?.circulatedAt ===
        ...
        correctedDetail.proofBundle.circulationRecord?.circulatedAt ===
          circulatedAt &&
        correctedDetail.proofBundle.circulationRecord?.circulationNote ===
          circulationNote,
      "Expected the original circulation record and note to remain immutable after note-reset logging.",
    );
    assert(
      correctedDetail.reporting?.circulationChronology?.correctionCount === 1 &&
        ...
        correctedDetail.reporting?.circulationChronology?.effectiveRecord
          ?.circulationNote === null &&
        correctedDetail.reporting?.circulationChronology?.latestCorrectionSummary?.includes(
          "circulationNote cleared to absent",
        ) &&
        ...
        correctedDetail.reporting?.circulationChronology?.latestCorrection
          ?.summary?.includes("circulationNote cleared to absent"),
      "Expected mission detail and proof bundle read models to expose original record plus corrected effective note absence.",
    );
    assert(
      correctedApprovalCard?.title?.includes("circulation corrected") &&
        correctedApprovalCard?.summary?.includes("append-only correction") &&
        correctedMissionListItem?.circulationRecord?.circulationNote ===
          circulationNote &&
        ...
        correctedMissionListItem?.circulationChronology?.correctionCount ===
          1 &&
        correctedMissionListItem?.circulationChronology?.latestCorrectionSummary?.includes(
          "circulationNote cleared to absent",
        ),
      "Expected approval cards and mission-list read models to surface board-circulation note-reset chronology.",
    );
    assert(
      ...
      JSON.stringify(
        {
          ...
          mode: "api_only_persisted_report_circulation_note_reset",
          company: {
            companyKey: fixture.companyKey,
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-board-packet-circulation-note-reset-smoke-"),
  );
