...
const DEFAULT_CREATED_BY = "collections-pressure-monitor-smoke";
  ...
  return {
    ...
    companies: {
      missing: buildCompanyFixture({
        companyKey: `local-collections-monitor-missing-${runTag}`,
        companyName: `Local Collections Monitor Missing ${input.runTag}`,
        originalFileName: `collections-monitor-trial-balance-${input.runTag}.csv`,
        sourceName: `Collections monitor missing ${input.runTag}`,
        text: [
      ...
      staleDataQuality: buildCompanyFixture({
        companyKey: `local-collections-monitor-stale-dq-${runTag}`,
        companyName: `Local Collections Monitor Stale DQ ${input.runTag}`,
        originalFileName: `collections-monitor-stale-dq-${input.runTag}.csv`,
        sourceName: `Collections monitor stale data quality ${input.runTag}`,
        text: [
          "customer_name,customer_id,currency,as_of,current,past_due,total",
          "Alpha Co,C-100,USD,2026-04-24,80.00,20.00,100.00",
          "Beta Co,C-200,USD,,80.00,20.00,100.00",
        ].join("\n"),
      }),
      coverageGap: buildCompanyFixture({
        companyKey: `local-collections-monitor-coverage-${runTag}`,
        companyName: `Local Collections Monitor Coverage ${input.runTag}`,
        originalFileName: `collections-monitor-coverage-${input.runTag}.csv`,
        sourceName: `Collections monitor coverage ${input.runTag}`,
        text: [
          "customer_name,customer_id,currency,as_of,over_90",
          "Partial Co,C-300,USD,2026-04-26,25.00",
        ].join("\n"),
      }),
      overdue: buildCompanyFixture({
        companyKey: `local-collections-monitor-overdue-${runTag}`,
        companyName: `Local Collections Monitor Overdue ${input.runTag}`,
        originalFileName: `collections-monitor-overdue-${input.runTag}.csv`,
        sourceName: `Collections monitor overdue ${input.runTag}`,
        text: [
          "customer_name,customer_id,currency,as_of,current,past_due,total",
          "Delta Co,C-400,USD,2026-04-26,20.00,80.00,100.00",
        ].join("\n"),
      }),
      clean: buildCompanyFixture({
        companyKey: `local-collections-monitor-clean-${runTag}`,
        companyName: `Local Collections Monitor Clean ${input.runTag}`,
        originalFileName: `collections-monitor-clean-${input.runTag}.csv`,
        sourceName: `Collections monitor clean ${input.runTag}`,
        text: [
          "customer_name,customer_id,currency,as_of,current,past_due,total",
    ...
    seed: {
      body: Buffer.from(
        `${JSON.stringify(
          {
            createdBy: DEFAULT_CREATED_BY,
            note: "Seed snapshot for the packaged F6C collections-pressure monitor smoke.",
            requestedBy: "collections_pressure_monitor_smoke",
            runTag: input.runTag,
          },
          null,
          2,
      ...
      originalFileName: `collections-pressure-monitor-seed-${input.runTag}.json`,
    },
  };
}
...
async function main() {
  ...
  try {
    ...
    const missingRun = await runCollectionsMonitor(app, {
    ...
    const staleRun = await runCollectionsMonitor(app, {
    ...
    const coverageRun = await runCollectionsMonitor(app, {
    ...
    const overdueRun = await runCollectionsMonitor(app, {
      companyKey: fixture.companies.overdue.companyKey,
      runKey: `overdue-${runTag}`,
    });
    const overdueRetry = await runCollectionsMonitor(app, {
      companyKey: fixture.companies.overdue.companyKey,
      runKey: `overdue-${runTag}`,
    });
    ...
    const cleanRun = await runCollectionsMonitor(app, {
    ...
    const latestOverdue = await injectJson(app, {
      expectedStatus: 200,
      method: "GET",
      url: `/monitoring/companies/${fixture.companies.overdue.companyKey}/collections-pressure/latest`,
    });

    if (latestOverdue.monitorResult?.id !== overdueRun.monitorResult.id) {
      throw new Error("Latest collections monitor read did not return the persisted result");
    }
      ...
      JSON.stringify(
        {
          ...
          idempotentRunKeyPreserved: overdueRun.monitorResult.id === overdueRetry.monitorResult.id,
...
async function syncCompanyFixture(app, input) {
  const created = await injectJson(app, {
    ...
    payload: {
      ...
      description: "Packaged F6C collections-pressure monitor smoke source.",
      kind: "dataset",
...
async function runCollectionsMonitor(app, input) {
  return injectJson(app, {
    expectedStatus: 201,
    method: "POST",
    payload: {
    ...
    url: `/monitoring/companies/${input.companyKey}/collections-pressure/run`,
  });
}
...
function assertAlertResult(run, input) {
  ...
  if (
    ...
    throw new Error(
      `Unexpected collections alert posture: ${JSON.stringify(run.monitorResult)}`,
    );
  }
  ...
  if (!run.alertCard) {
    throw new Error("Alerting collections monitor result did not include an alert card");
  }

  if (run.alertCard.monitorKind !== "collections_pressure") {
    throw new Error("Collections alert card did not preserve monitorKind");
  }
  ...
  if (run.alertCard.sourceLineageRefs.length !== run.monitorResult.sourceLineageRefs.length) {
  ...
  if (input.requireSourceLineage && run.monitorResult.sourceLineageRefs.length === 0) {
    throw new Error("Source-backed collections alert did not include lineage refs");
  }
}
...
function assertNoAlertResult(run, companyKey) {
  ...
  if (
    ...
    throw new Error(
      `Expected a non-alerting clean collections monitor result, received ${JSON.stringify(run)}`,
    );
  }

  if (run.monitorResult.sourceLineageRefs.length === 0) {
    throw new Error("Clean collections monitor result did not include source lineage refs");
  }

  if (run.monitorResult.proofBundlePosture.state !== "source_backed") {
    throw new Error(
      `Clean collections proof posture was ${run.monitorResult.proofBundlePosture.state}`,
    );
  }
...
function assertRequiredMonitorPosture(result) {
  ...
  if (
    ...
    throw new Error("Monitor result crossed an F6C runtime or action boundary");
  }
}

function assertIdempotentRetry(first, second) {
  if (first.monitorResult.id !== second.monitorResult.id) {
    throw new Error("Repeated collections monitor runKey created a duplicate result");
  }

  if (first.monitorResult.createdAt !== second.monitorResult.createdAt) {
    throw new Error("Repeated collections monitor runKey changed createdAt");
  }
}
...
function assertBoundaryCounts(before, after) {
  for (const key of Object.keys(before)) {
    if (before[key] !== after[key]) {
      throw new Error(
        `F6C collections monitor changed ${key}: before=${before[key]}, after=${after[key]}`,
      );
    }
  }
}
...
async function writeSeedSnapshot(seed) {
  const directory = await mkdtemp(
    join(tmpdir(), "pocket-cfo-collections-monitor-"),
  );
