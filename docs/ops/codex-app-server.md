...
`plans/FP-0055-monitor-demo-replay-and-stack-pack-foundation.md` is the active F6F implementation-ready contract.
F6F remains runtime-free: the planned demo replay should register checked-in source fixtures, run existing deterministic sync/wiki/monitor paths, compare expected outputs, and demonstrate the shipped cash-only handoff where applicable without creating runtime-Codex monitoring findings or investigation writeups.

The Codex App Server should not draft monitoring findings, run natural-language autonomous monitoring, create investigation writeups, send notifications, publish messages, create payment instructions, recommend vendor payments, interpret policy or legal obligations, or perform remediation in F6A, F6B, F6C, F6D, F6E, or F6F.
