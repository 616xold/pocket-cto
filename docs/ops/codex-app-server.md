...
The shipped F6A record is now `plans/FP-0050-monitoring-foundation-and-first-cash-posture-alert.md`. The shipped F6B record is now `plans/FP-0051-alert-to-investigation-mission-foundation.md`. Do not create runtime-codex monitoring behavior, notification delivery, automatic mission creation, or F6C and later monitor work from this doc alone; the next F6 thread should define F6C planning/docs before implementation.
