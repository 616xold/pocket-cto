...
The next later-F5 step is the active F5C4G correction-and-chronology contract: keep the original board `circulationRecord` immutable, append correction history on the existing `report_circulation` seam, derive a current effective circulation view plus chronology summary, and avoid any broader delivery, export, or runtime-codex widening.
