...
`plans/FP-0052-collections-pressure-monitor-foundation.md` now records the F6C implementation slice for exactly one deterministic `collections_pressure` monitor over stored receivables-aging or collections-posture Finance Twin state. Do not start broader later-F5 work, multi-monitor F6 work, F6D or later implementation, runtime-codex monitoring, delivery behavior, notifications, investigations for collections alerts, approvals, report conversion, or automatic mission creation from this doc.
...
6. run discovery, reporting, and monitoring work only through the current active Finance Plan; FP-0052 now records the shipped F6C collections-pressure monitor slice, and F6D needs a new Finance Plan before implementation starts
...
`plans/FP-0050-monitoring-foundation-and-first-cash-posture-alert.md` is the shipped first F6A implementation record. `plans/FP-0051-alert-to-investigation-mission-foundation.md` is the shipped first F6B implementation record. `plans/FP-0052-collections-pressure-monitor-foundation.md` is the F6C implementation record; do not reopen F4C2, repeat F5A through F5C4I, start F6D, or widen beyond one deterministic `collections_pressure` monitor without a new named Finance Plan.
