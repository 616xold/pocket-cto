...
- **V2G read-only ChatGPT App/MCP contract foundation**: shipped through FP-0087 as a local proof-only typed contract foundation for future read-only ChatGPT App/MCP wrapper concepts. It adds pure domain contracts, an exact read-only MCP allowlist, forbidden-tool and renamed-equivalent rejection, evidence query/fetch/source-coverage/capability-boundary fetch contracts, refusal posture, prompt-injection/privacy/no-runtime boundaries, deferred OAuth/submission/provider-certification boundaries, threat-model questions, focused specs, typed V2F boundary hardening, and one direct proof command. It adds no public app implementation, MCP server runtime, UI, routes, endpoints, Apps SDK iframe, OAuth, app submission, schema, migrations, package scripts, eval datasets, fixtures, sample data, public demo data, source-pack behavior, OpenAI API/model calls, hosted tools, vector/file-search integration, OCR, PageIndex, provider behavior, certification, delivery, deployment, external communications, source mutation, finance writes, generated product prose, runtime-Codex finance output, or autonomous action.

## Distribution And Precision Decisions

- ChatGPT App/MCP is worth keeping as a read-only evidence wrapper path after EvidenceIndex, V2B precision proof, the shipped FP-0082 local/internal tool contract, the shipped FP-0083 OSS/security/privacy/self-host/demo baseline, the shipped V2E bounded-orchestration posture, the shipped V2F no-real-finance-data benchmark/community posture, and the shipped FP-0087 local proof-only contract foundation. MCP server runtime, public ChatGPT App implementation, Apps SDK UI, remote deployment, OAuth, and app publication remain future implementation work.
- iOS is postponed.
