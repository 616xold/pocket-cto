...
- **V2I premium read-only ChatGPT App/MCP UI design-system readiness**: active through FP-0089 as docs-and-plan plus proof-gate compatibility only. It defines future design tokens, semantic color, spacing, typography, component taxonomy, evidence hierarchy, accessibility requirements, refusal-state grammar, interaction rules, copy style, and design QA gates without implementing UI, Apps SDK iframe/UI, endpoints, remote MCP, OAuth, app submission, OpenAI API/model calls, data files, package scripts, source mutation, finance writes, generated product prose, runtime-Codex finance output, or autonomous action.

## Distribution And Precision Decisions

- ChatGPT App/MCP is worth keeping as a read-only evidence wrapper path after EvidenceIndex, V2B precision proof, the shipped FP-0082 local/internal tool contract, the shipped FP-0083 OSS/security/privacy/self-host/demo baseline, the shipped V2E bounded-orchestration posture, the shipped V2F no-real-finance-data benchmark/community posture, the shipped FP-0087 local proof-only contract and descriptor/envelope foundation, the shipped FP-0088 premium UI/security readiness plan, and the active FP-0089 premium UI design-system readiness plan. MCP server runtime, public ChatGPT App implementation, Apps SDK UI, remote deployment, OAuth, and app publication remain future implementation work.
- iOS is postponed.
