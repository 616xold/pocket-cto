...
`plans/FP-0048-board-packet-circulation-actor-correction-and-chronology-hardening.md` is now the active next benchmark contract and narrows any remaining later-F5 board work to optional corrected `circulatedBy` plus derived effective actor chronology on that same seam only.
