...
`plans/FP-0052-collections-pressure-monitor-foundation.md` is the active F6C implementation-ready contract for exactly one `collections_pressure` monitor. Do not add a broader F6 benchmark dataset or start F6D and later implementation before a new named Finance Plan defines that next monitor scope.
