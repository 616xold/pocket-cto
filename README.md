...
`plans/FP-0048-board-packet-circulation-actor-correction-and-chronology-hardening.md` is now the active F5C4H contract and already records the first shipped actor-correction slice: the repo can append optional corrected `circulatedBy` on that same seam, preserve the immutable original circulation record, derive truthful effective-actor chronology, and stay deterministic, runtime-free, delivery-free, and board-packet-only. Keep any remaining work inside that narrow FP-0048 scope rather than starting broader later-F5 or F6 work.
