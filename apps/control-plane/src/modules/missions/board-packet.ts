...
const BOARD_PACKET_INTAKE_COMPILER: MissionCompilationResult = {
  compilerName: "typed-board-packet-intake",
...
type BoardPacketCreationSource = {
...
export async function buildBoardPacketMissionCreationInput(
  rawInput: CreateBoardPacketMissionInput,
  deps: {
  ...
  const input = CreateBoardPacketMissionInputSchema.parse(rawInput);
  const source = await resolveBoardPacketCreationSource(
    input.sourceReportingMissionId,
    deps,
  );
  const reportingRequest = buildBoardPacketMissionInput(source);
  const spec = buildBoardPacketMissionSpec(reportingRequest);

  return {
    compilation: {
      ...BOARD_PACKET_INTAKE_COMPILER,
      spec,
...
async function resolveBoardPacketCreationSource(
  sourceReportingMissionId: string,
  deps: {
    missionRepository: Pick<
      MissionRepository,
...
): Promise<BoardPacketCreationSource> {
  ...
  if (sourceMission.status !== "succeeded") {
    throw createInvalidRequestError(
      "sourceReportingMissionId",
      `Reporting mission ${sourceReportingMissionId} must have status \`succeeded\` before draft board-packet reporting can start.`,
    );
  }
  ...
  if (!reportingView.financeMemo || !reportingView.evidenceAppendix) {
    throw createInvalidRequestError(
      "sourceReportingMissionId",
      `Reporting mission ${sourceReportingMissionId} must store both finance_memo and evidence_appendix artifacts before draft board-packet reporting can start.`,
    );
  }
...
function buildBoardPacketMissionInput(
  input: BoardPacketCreationSource,
): ReportingMissionInput {
  return {
...
function buildBoardPacketMissionSpec(input: ReportingMissionInput): MissionSpec {
  return {
    type: "reporting",
    title: buildBoardPacketMissionTitle(input),
    objective: buildBoardPacketMissionObjective(input),
    repos: [],
    constraints: {
      mustNot: [
        "do not invoke the codex runtime",
        "do not read generic chat text or freeform packet prompts",
    ...
    acceptance: [
      "persist one draft board_packet artifact",
...
function buildBoardPacketMissionTitle(input: ReportingMissionInput) {
  const companySegment = input.companyKey ?? "the scoped company";

  if (input.questionKind) {
    return `Draft board packet for ${companySegment} from ${readFinanceDiscoveryQuestionKindLabel(input.questionKind).toLowerCase()} reporting`;
  }

  return `Draft board packet for ${companySegment}`;
}

function buildBoardPacketMissionObjective(input: ReportingMissionInput) {
  return `Compile one draft board packet from completed reporting mission ${input.sourceReportingMissionId} and its stored finance memo plus evidence appendix only.`;
}
