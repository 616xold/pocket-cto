...
const LENDER_UPDATE_INTAKE_COMPILER: MissionCompilationResult = {
  compilerName: "typed-lender-update-intake",
...
type LenderUpdateCreationSource = {
...
export async function buildLenderUpdateMissionCreationInput(
  rawInput: CreateLenderUpdateMissionInput,
  deps: {
  ...
  const input = CreateLenderUpdateMissionInputSchema.parse(rawInput);
  const source = await resolveLenderUpdateCreationSource(
    input.sourceReportingMissionId,
    deps,
  );
  const reportingRequest = buildLenderUpdateMissionInput(source);
  const spec = buildLenderUpdateMissionSpec(reportingRequest);

  return {
    compilation: {
      ...LENDER_UPDATE_INTAKE_COMPILER,
      spec,
...
async function resolveLenderUpdateCreationSource(
  sourceReportingMissionId: string,
  deps: {
    missionRepository: Pick<
      MissionRepository,
...
): Promise<LenderUpdateCreationSource> {
  ...
  if (sourceMission.status !== "succeeded") {
    throw createInvalidRequestError(
      "sourceReportingMissionId",
      `Reporting mission ${sourceReportingMissionId} must have status \`succeeded\` before draft lender-update reporting can start.`,
    );
  }
  ...
  if (!reportingView.financeMemo || !reportingView.evidenceAppendix) {
    throw createInvalidRequestError(
      "sourceReportingMissionId",
      `Reporting mission ${sourceReportingMissionId} must store both finance_memo and evidence_appendix artifacts before draft lender-update reporting can start.`,
    );
  }
...
function buildLenderUpdateMissionInput(
  input: LenderUpdateCreationSource,
): ReportingMissionInput {
  return {
...
function buildLenderUpdateMissionSpec(input: ReportingMissionInput): MissionSpec {
  return {
    type: "reporting",
    title: buildLenderUpdateMissionTitle(input),
    objective: buildLenderUpdateMissionObjective(input),
    repos: [],
    constraints: {
      mustNot: [
    ...
    acceptance: [
      "persist one draft lender_update artifact",
...
function buildLenderUpdateMissionTitle(input: ReportingMissionInput) {
  const companySegment = input.companyKey ?? "the scoped company";

  if (input.questionKind) {
    return `Draft lender update for ${companySegment} from ${readFinanceDiscoveryQuestionKindLabel(input.questionKind).toLowerCase()} reporting`;
  }

  return `Draft lender update for ${companySegment}`;
}

function buildLenderUpdateMissionObjective(input: ReportingMissionInput) {
  return `Compile one draft lender update from completed reporting mission ${input.sourceReportingMissionId} and its stored finance memo plus evidence appendix only.`;
}
