...
`plans/FP-0049-board-packet-circulation-note-reset-and-effective-record-hardening.md` is now the shipped F5C4I record for explicit append-only `circulationNote` clear-to-absent semantics only on this same approvals seam. Do not widen into a second subsystem, a new approval kind, or delivery behavior.
