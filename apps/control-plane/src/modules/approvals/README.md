...
The next later-F5 follow-on is now the active `plans/FP-0047-board-packet-circulation-record-correction-and-chronology-foundation.md` contract: keep the original board `circulationRecord` immutable, append correction history on the existing `report_circulation` seam, derive a current effective circulation view plus chronology summary, and avoid any broader delivery or export widening.
