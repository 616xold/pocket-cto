...
import type { CompiledBoardPacketArtifacts, SourceReportingBundle } from "./types";

export function compileBoardPacketArtifacts(
  ...
  return {
    ...
    boardPacket: {
      ...
      bodyMarkdown: buildBoardPacketMarkdown(source),
    },
  };
}

function buildBoardPacketMarkdown(source: SourceReportingBundle) {
  const financeMemo = source.sourceReportingView.financeMemo;
...
function buildPacketSummary(source: SourceReportingBundle) {
  ...
  return `Draft board packet for ${companySegment} from the completed ${questionSegment} reporting mission. ${financeMemo.memoSummary}`;
}
